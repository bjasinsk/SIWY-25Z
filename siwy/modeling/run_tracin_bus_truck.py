...
from siwy.datasets.BusTruckConfig import CLASS_TO_IDX
...
def plot_tracin_top_contributors(run, train_loader, test_loader, tracin_matrix, test_indices, top_k=5):
    ...
    for test_idx in test_indices:
        ...
        for i, idx in enumerate(top_indices):
            ...
            axs[i + 1].axis("off")
        plt.tight_layout()
        fig_path = FIGURES_DIR / f"tracin_bustruck_{test_idx}.png"
        plt.savefig(fig_path)
        run.log({f"tracin_bustruck_{test_idx}": wandb.Image(fig)})
        plt.close(fig)
...
def main(dataset="bus-and-truck-easy-train", batch_size=5, num_classes=3, lr=0.001, epochs=None):
    if epochs is None:
        epochs = [0, 1, 2, 4, 6, 8]
    ...
    bus_truck_ds = torch.load(PROCESSED_DATA_DIR / "bus-and-truck-easy-train.pt", weights_only=False)
    airplane_ds = torch.load(AIRPLANES_DATASET_PATH, weights_only=False)
    logger.debug(f"Airplanes dataset: {airplane_ds}")
    train_ds = bus_truck_ds
    ...
    matrix_path = MODELS_DIR / "tracin_score_matrix_bus_and_truck.pt"
...
def plot_no_train(batch_size):
    ...
    plot_tracin_top_contributors(
        ...
        train_loader,
        test_loader,
        tracin_matrix=torch.load("tracin_score_matrix_bus_and_truck.pt"),
        test_idx=0,
        top_k=TOP_K,
    )
    run.finish()
