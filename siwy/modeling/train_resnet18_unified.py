...
from siwy.datasets.common import DEFAULT_TRANSFORM
from siwy.modeling.common_configs import TRAINING_CONFIGS
...
def train_model(
    ...
    TRAINING_PATH = PROCESSED_DATA_DIR / dataset_name / DATETIME
        ...
        if loader_val is not None:
            ...
            if acc > best_acc:
                ...
                artifact = wandb.Artifact(
                    name=f"{dataset_name}-{DATETIME}-model-{model_id}-epoch-{ep}",
                    type="model",
                )
...
def main(
    ...
    dataset: str = typer.Option("dog-and-cat", help="Dataset: dog-and-cat | bus-and-truck | horse-and-elephant"),
    ...
    with wandb.init(project=f"{WANDB_PROJECT}", job_type="training") as run:
        if config["split_mode"] == "separate":
            ...
            train_ds = torch.load(Path(train_artifact_path) / f"{config['train_dataset']}.pt", weights_only=False)[
                "train"
            ]

            val_artifact = run.use_artifact(WANDB_DATASET_PATH(config["val_dataset"]), type="dataset")
            val_artifact_path = val_artifact.download(PROCESSED_DATA_DIR)
            val_ds = torch.load(Path(val_artifact_path) / f"{config['val_dataset']}.pt", weights_only=False)["val"]
        else:
            # Load combined dataset and split internally
            artifact = run.use_artifact(WANDB_DATASET_PATH(config["dataset_name"]), type="dataset")
            artifact_path = artifact.download(PROCESSED_DATA_DIR)
            ds = torch.load(Path(artifact_path) / f"{config['dataset_name']}.pt", weights_only=False)
            train_ds = ds["train"]
            val_ds = ConcatDataset([ds["val"], ds["test"]])
        ...
        for i in tqdm(range(1), desc="Training models.."):
            ...
            model = train_model(
                run, model, loader_for_training, loader_for_validation, model_id=i, dataset_name=dataset
            )
...
if __name__ == "__main__":
    # app()
    main(model="resnet18-pretrained", dataset="dog-and-cat", batch_size=32, num_classes=3)
